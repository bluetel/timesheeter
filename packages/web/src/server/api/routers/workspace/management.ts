import { createWorkspaceSchema, membershipRoleSchema, updateWorkspaceSchema } from '@timesheeter/web/lib';
...
export const managementRouter = createTRPCRouter({
  ...
  createWorkspace: protectedProcedure.input(createWorkspaceSchema).mutation(async ({ ctx, input }) => {
    ...
    if (invitationUsers.some((user) => input.memberships.some((membership) => membership.userId === user.id))) {
      throw new TRPCError({
        code: 'BAD_REQUEST',
  ...
  updateWorkspace: protectedProcedure.input(updateWorkspaceSchema).mutation(async ({ ctx, input }) => {
    ...
    const { memberships: oldMemberships, invitations: oldInvitations } = workspace;
    ...
    if (
      memberships &&
      invitationUsers.some((user) => memberships.some((membership) => membership.userId === user.id))
    ) {
      throw new TRPCError({
        code: 'BAD_REQUEST',
        message: 'Invited users cannot already be members',
    ...
    const membershipsToDelete =
      memberships !== undefined
        ? oldMemberships.filter(
            (oldMembership) => !memberships.find((membership) => membership.userId === oldMembership.userId)
          )
        : undefined;
    ...
    const membershipsToUpdate =
      memberships !== undefined
        ? memberships.filter((membership) =>
            oldMemberships.find((oldMembership) => oldMembership.userId === membership.userId)
          )
        : undefined;
    ...
    const invitationsToDelete =
      invitations !== undefined
        ? oldInvitations.filter(
            (oldInvitation) => !invitations.find((invitation) => invitation === oldInvitation.email)
          )
        : undefined;
    ...
    const invitationsToCreate =
      invitations !== undefined
        ? invitations.filter(
            (invitation) => !oldInvitations.find((oldInvitation) => oldInvitation.email === invitation)
          )
        : undefined;
    ...
    const updatedWorkspace = await ctx.prisma.workspace.update({
      ...
      data: {
        ...rest,
        memberships:
          membershipsToUpdate !== undefined && membershipsToDelete !== undefined
            ? {
...
const authorize = async ({
  ...
  const membership = workspace.memberships.find((membership) => membership.userId === userId);
